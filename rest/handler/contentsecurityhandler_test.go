...
func TestContentSecurityHandlerUnsignedCallback(t *testing.T) {
	keyFile, err := createTempFile(priKey)
	defer os.Remove(keyFile)
	...
	handler := contentSecurityHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { fmt.Print("123") }))
...
func TestContentSecurityHandlerUnsignedCallbackWrongTime(t *testing.T) {
	keyFile, err := createTempFile(priKey)
	defer os.Remove(keyFile)
...
func buildRequest(rs requestSettings) (*http.Request, error) {
	...
	if rs.crypt && rs.body != nil {
		...
		io.Copy(&buf, rs.body)
		bodyBytes, _ := codec.EcbEncrypt(key, buf.Bytes())
...
const literal_6842 = "http://localhost/a/b?c=d&e=f"
