...
func TestLogHandler(t *testing.T) {
	...
	for _, logHandler := range handlers {
		req := httptest.NewRequest(http.MethodGet, literal_0782, http.NoBody)
		handler := logHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set(literal_4185, "test")
		...
		assert.Equal(t, "test", resp.Header().Get(literal_4185))
...
func TestLogHandlerVeryLong(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, literal_0782, &buf)
	handler := LogHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		internal.LogCollectorFromContext(r.Context()).Append("anything")
...
func TestDetailedLogHandlerHijack(t *testing.T) {
	resp := httptest.NewRecorder()
...
func TestDumpRequest(t *testing.T) {
	const errMsg = "error"
	r := httptest.NewRequest(http.MethodGet, literal_0782, http.NoBody)
...
const literal_0782 = "http://localhost"

const literal_4185 = "X-Test"
