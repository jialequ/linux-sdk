...
func (h *EventHandler) OnUpdate(oldObj, newObj any) {
	...
	if !ok {
		logx.Errorf(literal_5408, oldObj)
		return
	}

	newEndpoints, ok := newObj.(*v1.Endpoints)
	if !ok {
		logx.Errorf(literal_5408, newObj)
		return
	}
...
const literal_5408 = "%v is not an object with type *v1.Endpoints"
