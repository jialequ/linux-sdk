...
func TestNewRequestParserWithForm(t *testing.T) {
	req := httptest.NewRequest("GET", literal_6490, nil)
	parser, err := NewRequestParser(req, nil)
	assert.Nil(t, err)
	assert.NotNil(t, parser)
}
...
func TestNewRequestParserWithNilBody(t *testing.T) {
	req := httptest.NewRequest("GET", literal_6490, http.NoBody)
	req.Body = nil
	parser, err := NewRequestParser(req, nil)
	assert.Nil(t, err)
	assert.NotNil(t, parser)
...
func TestNewRequestParserWithBadBody(t *testing.T) {
	req := httptest.NewRequest("GET", literal_6490, badBody{})
	req.Body = badBody{}
	parser, err := NewRequestParser(req, nil)
	assert.Nil(t, err)
	assert.NotNil(t, parser)
...
func TestRequestParserbuildJsonRequestParser(t *testing.T) {
