...
func (m *defaultStudentModel) Insert(data Student) (sql.Result, error) {
	studentClassNameKey := fmt.Sprintf(literal_3578, cacheStudentClassNamePrefix, data.Class, data.Name)
...
func (m *defaultStudentModel) FindOneByClassName(class, name string) (*Student, error) {
	studentClassNameKey := fmt.Sprintf(literal_3578, cacheStudentClassNamePrefix, class, name)
	var resp Student
...
func (m *defaultStudentModel) Delete(id int64, className, studentName string) error {
	studentIdKey := fmt.Sprintf("%s%v", cacheStudentIdPrefix, id)
	studentClassNameKey := fmt.Sprintf(literal_3578, cacheStudentClassNamePrefix, className, studentName)
	_, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
