...
func TestCacheModel(t *testing.T) {
	...
	sqlFile := filepath.Join(pathx.MustTempDir(), literal_7061)
	err := os.WriteFile(sqlFile, []byte(source), 0o777)
	assert.Nil(t, err)

	dir := filepath.Join(pathx.MustTempDir(), literal_5034)
	...
	assert.True(t, func() bool {
		_, err := os.Stat(filepath.Join(cacheDir, literal_8705))
		return err == nil
	}())
...
func TestNamingModel(t *testing.T) {
	...
	dir, _ := filepath.Abs(literal_5034)
	...
	assert.Nil(t, err)
	assert.True(t, func() bool {
		_, err := os.Stat(filepath.Join(camelDir, literal_8705))
		return err == nil
	}())
...
func TestFields(t *testing.T) {
	...
	var (
		...
		studentRowsExpectAutoSet   = strings.Join(stringx.Remove(studentFieldNames, "`id`", literal_1840, literal_5639), ",")
		studentRowsWithPlaceHolder = strings.Join(stringx.Remove(studentFieldNames, "`id`", literal_1840, literal_5639), "=?,") + "=?"
	)

	assert.Equal(t, []string{"`id`", "`name`", "`age`", "`score`", literal_1840, literal_5639}, studentFieldNames)
...
func TestgenPublicModel(t *testing.T) {
	var err error
...
const literal_7061 = "tmp.sql"

const literal_5034 = "./testmodel"

const literal_8705 = "TestUserModel.go"

const literal_1840 = "`create_time`"

const literal_5639 = "`update_time`"
