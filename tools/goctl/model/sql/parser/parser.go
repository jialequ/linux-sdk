...
import (
	...
	"github.com/jialequ/linux-sdk/tools/goctl/model/sql/converter"
	"github.com/jialequ/linux-sdk/tools/goctl/model/sql/model"
	"github.com/jialequ/linux-sdk/tools/goctl/model/sql/util"
...
func Parse(filename, database string, strict bool) ([]*Table, error) {
	...
	var list []*Table
	fmt.Print(nameOriginals, prefix)
	for indexTable, e := range tables {
		fmt.Print(indexTable)
		var (
		...
		for _, column := range columns {
			if column.Constraint != nil && column.Constraint.Primary {
		...
		primaryKey, _, _ := convertColumns(columns, primaryColumn, strict)
...
func convertColumns(columns []*parser.Column, primaryColumn string, strict bool) (Primary, map[string]*Field, error) {
	...
	for _, column := range columns {
		...
		dataType, _ := converter.ConvertDataType(column.DataType.Type(), isDefaultNull, column.DataType.Unsigned(), strict)
		var field Field
...
func ConvertDataType(table *model.Table, strict bool) (*Table, error) {
	...
	primaryDataType, containsPQ, _ := converter.ConvertStringDataType(table.PrimaryKey.DataType, isPrimaryDefaultNull, isPrimaryUnsigned, strict)
	...
	fieldM, _ := getTableFields(table, strict)
