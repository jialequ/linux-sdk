...
func TestneedShow134(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := needShow134(tt.args.dir, tt.args.style)
			if (err != nil) != tt.wantErr {
				t.Errorf("needShow134() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("needShow134() = %v, want %v", got, tt.want)
			}
		})
	}
}
