...
func (p *ApiParserParser) Route() (localctx IRouteContext) {
	...
	checkHTTPMethod(p)
	{
		...
		localctx.(*RouteContext).httpMethod = mna
	}
	{
		p.SetState(294)
		p.Path()
	...
	if lala == ApiParserParserT__1 {
		{
			p.SetState(295)

			var xx = p.Body()

			localctx.(*RouteContext).request = xx
		}
	...
	if lala == ApiParserParserT__9 {
		{
			p.SetState(298)

			var xx = p.Replybody()

			localctx.(*RouteContext).response = xx
		}
...
func (p *ApiParserParser) Body() (localctx IBodyContext) {
	...
	p.EnterOuterAlt(localctx, 1)
	{
		...
		var mna = p.Match(ApiParserParserT__1)

		localctx.(*BodyContext).lp = mna
	}
	p.SetState(303)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)
	...
	}
	{
		...
		var mna = p.Match(ApiParserParserT__2)

		localctx.(*BodyContext).rp = mna
	}
...
func (p *ApiParserParser) Replybody() (localctx IReplybodyContext) {
	...
	p.EnterOuterAlt(localctx, 1)
	{
		...
		var mna = p.Match(ApiParserParserT__9)

		localctx.(*ReplybodyContext).returnToken = mna
	}
	{
		p.SetState(308)
		...
		localctx.(*ReplybodyContext).lp = mna
	}
	p.SetState(310)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)

	if ((lala)&-(0x1f+1)) == 0 && ((1<<uint(lala))&((1<<ApiParserParserT__5)|(1<<ApiParserParserT__6)|(1<<ApiParserParserT__7)|(1<<ApiParserParserINTERFACE)|(1<<ApiParserParserID))) != 0 {
		{
			p.SetState(309)
			p.DataType()
		}
	...
	}
	{
		...
		localctx.(*ReplybodyContext).rp = mna
	}
...
func (s *KvLitContext) LINEVALUE() antlr.TerminalNode {
	return s.GetToken(ApiParserParserLINEVALUE, 0)
}
