...
func (p *ApiParserParser) TypeStruct() (localctx ITypeStructContext) {
	...
	checkKeyword(p)
	{
		...
		var mna = p.Match(ApiParserParserID)

		localctx.(*TypeStructContext).structName = mna
	}
	p.SetState(158)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)

	if lala == ApiParserParserID {
		{
			p.SetState(157)

			var mna = p.Match(ApiParserParserID)

			localctx.(*TypeStructContext).structToken = mna
		}
	...
	}
	{
		...
		var mna = p.Match(ApiParserParserT__3)

		localctx.(*TypeStructContext).lbrace = mna
	}
	p.SetState(164)
	p.GetErrorHandler().Sync(p)
	alt = p.GetInterpreter().AdaptivePredict(p.GetTokenStream(), 10, p.GetParserRuleContext())

	for alt != 2 && alt != antlr.ATNInvalidAltNumber {
		if alt == 1 {
			{
				p.SetState(161)
				p.Field()
	...
	}
	{
		...
		var mna = p.Match(ApiParserParserT__4)

		localctx.(*TypeStructContext).rbrace = mna
	}
...
func (p *ApiParserParser) TypeAlias() (localctx ITypeAliasContext) {
	...
	checkKeyword(p)
	{
		...
		localctx.(*TypeAliasContext).alias = mna
	}
	p.SetState(172)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)

	if lala == ApiParserParserT__0 {
		{
			p.SetState(171)

			var mna = p.Match(ApiParserParserT__0)

			localctx.(*TypeAliasContext).assign = mna
		}
...
func (p *ApiParserParser) TypeBlockStruct() (localctx ITypeBlockStructContext) {
	...
	checkKeyword(p)
	{
		...
		localctx.(*TypeBlockStructContext).structName = mna
	}
	p.SetState(179)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)
	...
	if lala == ApiParserParserID {
		{
			...
			localctx.(*TypeBlockStructContext).structToken = mna
		}
	...
	}
	{
		...
		localctx.(*TypeBlockStructContext).lbrace = mna
	}
	p.SetState(185)
	p.GetErrorHandler().Sync(p)
	alt = p.GetInterpreter().AdaptivePredict(p.GetTokenStream(), 13, p.GetParserRuleContext())
	...
	}
	{
		...
		localctx.(*TypeBlockStructContext).rbrace = mna
	}
