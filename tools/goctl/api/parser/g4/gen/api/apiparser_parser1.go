...
func (p *ApiParserParser) SyntaxLit() (localctx ISyntaxLitContext) {
	...
	match(p, "syntax")
	{
		...
		localctx.(*SyntaxLitContext).syntaxToken = mna
	}
	{
		p.SetState(91)
		...
		localctx.(*SyntaxLitContext).assign = mna
	}
	checkVersion(p)
	{
		p.SetState(93)

		var mna = p.Match(ApiParserParserSTRING)

		localctx.(*SyntaxLitContext).version = mna
	}
...
func (p *ApiParserParser) ImportLit() (localctx IImportLitContext) {
	...
	match(p, "import")
	{
		...
		localctx.(*ImportLitContext).importToken = mna
	}
	{
		p.SetState(101)
		p.ImportValue()
...
func (p *ApiParserParser) ImportBlock() (localctx IImportBlockContext) {
	...
	match(p, "import")
	{
		...
		localctx.(*ImportBlockContext).importToken = mna
	}
	{
		p.SetState(105)
	...
	for ok := true; ok; ok = lala == ApiParserParserSTRING {
		{
			p.SetState(106)
			p.ImportBlockValue()
		}
