...
func checkKeyword(p *ApiParserParser) {
	...
	if IsGolangKeyWord(v) {
		notifyErrorListeners(p, fmt.Sprintf(literal_6792, v))
	}
}
...
func isNormal(p *ApiParserParser) bool { //NOSONAR
	...
	if len(list) == 1 {
		...
		if IsGolangKeyWord(t) {
			notifyErrorListeners(p, fmt.Sprintf(literal_6792, t))
		}
	}
	if len(list) > 1 {
		if list[0] == "*" {
...
func isInterface(p *ApiParserParser) {
	v := getCurrentTokenText(p)
	if IsGolangKeyWord(v) {
		notifyErrorListeners(p, fmt.Sprintf("expecting ID: %s", v))
	}
}
...
const literal_6792 = "expecting ID, found golang keyword: '%s'"
