...
func (p *ApiParserParser) AtServer() (localctx IAtServerContext) {
	...
	}
	{
		...
		localctx.(*AtServerContext).lp = mna
	}
	p.SetState(247)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)

	for ok := true; ok; ok = lala == ApiParserParserID {
		{
			p.SetState(246)
			p.KvLit()
		}
	...
	}
	{
		...
		localctx.(*AtServerContext).rp = mna
	}
...
func (p *ApiParserParser) ServiceApi() (localctx IServiceApiContext) {
	...
	match(p, "service")
	{
		...
		localctx.(*ServiceApiContext).serviceToken = mna
	}
	{
		p.SetState(255)
		p.ServiceName()
	}
	{
		...
		localctx.(*ServiceApiContext).lbrace = mna
	}
	p.SetState(260)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)

	for ((lala)&-(0x1f+1)) == 0 && ((1<<uint(lala))&((1<<ApiParserParserATDOC)|(1<<ApiParserParserATHANDLER)|(1<<ApiParserParserATSERVER))) != 0 {
		{
			p.SetState(257)
			p.ServiceRoute()
		}
	...
	}
	{
		...
		localctx.(*ServiceApiContext).rbrace = mna
	}
...
func (p *ApiParserParser) AtDoc() (localctx IAtDocContext) {
	...
	if lala == ApiParserParserT__2 {
		{
			p.SetState(286)

			var mna = p.Match(ApiParserParserT__2)

			localctx.(*AtDocContext).rp = mna
		}
