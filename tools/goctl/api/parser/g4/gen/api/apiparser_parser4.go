...
func (p *ApiParserParser) TypeBlockAlias() (localctx ITypeBlockAliasContext) {
	...
	checkKeyword(p)
	{
		...
		localctx.(*TypeBlockAliasContext).alias = mna
	}
	p.SetState(193)
	...
	if lala == ApiParserParserT__0 {
		{
			...
			localctx.(*TypeBlockAliasContext).assign = mna
		}
...
func (s *NormalFieldContext) RAWSTRING() antlr.TerminalNode {
	return s.GetToken(ApiParserParserRAWSTRING, 0)
}
...
func (p *ApiParserParser) NormalField() (localctx INormalFieldContext) {
	...
	checkKeyword(p)
	{
		...
		localctx.(*NormalFieldContext).fieldName = mna
	}
	{
		p.SetState(204)
		p.DataType()
	...
	if p.GetInterpreter().AdaptivePredict(p.GetTokenStream(), 16, p.GetParserRuleContext()) == 1 {
		{
			...
			var mna = p.Match(ApiParserParserRAWSTRING)

			localctx.(*NormalFieldContext).tag = mna
		}
...
func (p *ApiParserParser) AnonymousFiled() (localctx IAnonymousFiledContext) {
	...
	if lala == ApiParserParserT__5 {
		{
			p.SetState(208)

			var mna = p.Match(ApiParserParserT__5)

			localctx.(*AnonymousFiledContext).star = mna
		}
