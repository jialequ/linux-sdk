...
func (p *ApiParserParser) InfoSpec() (localctx IInfoSpecContext) {
	...
	match(p, "info")
	{
		...
		localctx.(*InfoSpecContext).infoToken = mna
	}
	{
		p.SetState(120)
		...
		localctx.(*InfoSpecContext).lp = mna
	}
	p.SetState(122)
	...
	}
	{
		...
		localctx.(*InfoSpecContext).rp = mna
	}
...
func (p *ApiParserParser) TypeLit() (localctx ITypeLitContext) {
	...
	match(p, "type")
	{
		...
		localctx.(*TypeLitContext).typeToken = mna
	}
	{
		p.SetState(134)
		p.TypeLitBody()
...
func (p *ApiParserParser) TypeBlock() (localctx ITypeBlockContext) {
	...
	match(p, "type")
	{
		...
		localctx.(*TypeBlockContext).typeToken = mna
	}
	{
		p.SetState(138)
		...
		localctx.(*TypeBlockContext).lp = mna
	}
	p.SetState(142)
	p.GetErrorHandler().Sync(p)
	lala = p.GetTokenStream().LA(1)

	for lala == ApiParserParserID {
		{
			p.SetState(139)
			p.TypeBlockBody()
		}
	...
	}
	{
		...
		localctx.(*TypeBlockContext).rp = mna
	}
