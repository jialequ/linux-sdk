...
func (p *ApiParserParser) KvLit() (localctx IKvLitContext) {
	...
	p.EnterOuterAlt(localctx, 1)
	{
		...
		localctx.(*KvLitContext).key = mna
	}
	checkKeyValue(p)
	{
		p.SetState(316)

		var mna = p.Match(ApiParserParserLINEVALUE)

		localctx.(*KvLitContext).value = mna
	}
...
func (p *ApiParserParser) Path() (localctx IPathContext) { //NOSONAR
	...
	case 1:
		...
		for ok := true; ok; ok = lala == ApiParserParserT__11 || lala == ApiParserParserT__12 {
			p.SetState(341)
			p.GetErrorHandler().Sync(p)
...
func (p *ApiParserParser) PathItem() (localctx IPathItemContext) {
	...
	for ok := true; ok; ok = lala == ApiParserParserID || lala == ApiParserParserLetterOrDigit {
		{
			p.SetState(348)
			lala = p.GetTokenStream().LA(1)

			if !(lala == ApiParserParserID || lala == ApiParserParserLetterOrDigit) {
...
func (p *ApiParserParser) Sempred(localctx antlr.RuleContext, ruleIndex, predIndex int) bool {
	...
	case 18:
		...
		return p.FieldSempred(t, predIndex)
...
func (p *ApiParserParser) FieldSempred(localctx antlr.RuleContext, predIndex int) bool {
	switch predIndex {
	case 0:
		return isNormal(p)
