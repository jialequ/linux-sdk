...
func TestRoute(t *testing.T) {
	...
	t.Run("normal", func(t *testing.T) {
		...
		assert.True(t, route.Equal(&ast.Route{
			...
			Path:   ast.NewTextExpr(literal_4235),
			Req: &ast.Body{
				Lp:   ast.NewTextExpr("("),
				Rp:   ast.NewTextExpr(")"),
				Name: &ast.Literal{Literal: ast.NewTextExpr("Foo")},
		...
		assert.True(t, route.Equal(&ast.Route{
			...
			Path:   ast.NewTextExpr(literal_4235),
			DocExpr: []ast.Expr{
				ast.NewTextExpr(literal_7918),
			},
			CommentExpr: ast.NewTextExpr(literal_6530),
		}))
	})
...
func TestAtHandler(t *testing.T) {
	...
	t.Run("normal", func(t *testing.T) {
		...
		assert.True(t, atHandler.Equal(&ast.AtHandler{
			AtHandlerToken: ast.NewTextExpr(literal_9612),
...
func TestServiceApi(t *testing.T) {
	...
	t.Run("normal", func(t *testing.T) {
		...
		assert.True(t, api.Equal(&ast.ServiceApi{
			...
			Name:         ast.NewTextExpr(literal_7506),
...
const literal_4235 = "/foo/foo-bar/:bar"

const literal_7918 = "// foo"

const literal_6530 = "// bar"

const literal_9612 = "@handler"

const literal_7506 = "foo-api"
