...
func (v *ApiVisitor) acceptService(root, final *Api) { //NOSONAR
			...
			if route.AtServer != nil {
				...
				for _, kv := range route.AtServer.Kv {
					if _, ok := atServerM[kv.Key.Text()]; ok {
						v.panic(kv.Key, fmt.Sprintf(literal_2105, kv.Key.Text()))
					}
	...
	if root.Info != nil {
		...
		for _, value := range root.Info.Kvs {
			if _, ok := infoM[value.Key.Text()]; ok {
				v.panic(value.Key, fmt.Sprintf(literal_2105, value.Key.Text()))
			}
...
const literal_2105 = "duplicate key '%s'"
