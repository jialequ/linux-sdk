...
import (
	...
	"github.com/jialequ/linux-sdk/tools/goctl/api/parser"
	"github.com/jialequ/linux-sdk/tools/goctl/api/util"
	"github.com/jialequ/linux-sdk/tools/goctl/pkg/env"
	apiF "github.com/jialequ/linux-sdk/tools/goctl/pkg/parser/api/format"
...
func apiFormat(data string, skipCheckDeclare bool, filename ...string) (string, error) {
	...
	for s.Scan() {
		...
		if tapCount == 0 {
			ft, _ := formatGoTypeDef(line, s, &builder)
...
func formatGoTypeDef(line string, scanner *bufio.Scanner, builder *strings.Builder) (bool, error) {
	...
	if strings.HasPrefix(noCommentLine, "type") && (strings.HasSuffix(noCommentLine, leftParenthesis) ||
		...
		for scanner.Scan() {
			...
			if tokenCount == 0 {
				ts, _ := format.Source([]byte(typeBuilder.String()))
