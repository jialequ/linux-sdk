...
import (
	...
	"github.com/jialequ/linux-sdk/tools/goctl/pkg/parser/api/scanner"
...
func (p *Parser) parsePathExpr() *ast.PathExpr {
	...
	for p.curTokenIsNotEof() &&
		...
		p.notExpectPeekTokenGotComment(p.curTokenNode().PeekFirstLeadingComment(), token.COLON, token.IDENT, token.INT)

		// token ':' or IDENT
		p.notExpectPeekToken(token.COLON, token.IDENT, token.INT)

		p.notExpectPeekTokenGotComment(p.curTokenNode().PeekFirstLeadingComment(), token.COLON)
		...
		p.notExpectPeekToken(token.QUO, token.LPAREN, token.Returns, token.AT_DOC, token.AT_HANDLER, token.SEMICOLON, token.RBRACE)
	}
...
func (p *Parser) parsePathItem() []token.Token {
	...
	for p.curTokenIsNotEof() &&
		...
		if p.peekTokenIs(token.SUB) {
			...
			p.advanceIfPeekTokenIs(token.IDENT)
			list = append(list, p.curTok)
		} else {
			if p.peekTokenIs(token.LPAREN, token.Returns, token.AT_DOC, token.AT_HANDLER, token.SEMICOLON, token.RBRACE) {
				return list
...
func (p *Parser) parseElemExpr() *ast.ElemExpr {
	...
	p.advanceIfPeekTokenIs(token.IDENT, token.MUL)
	if p.curTokenIsKeyword() {
		return nil
	}
	...
	if p.curTokenIs(token.MUL) {
	...
	} else if p.peekTok.Line() > identNode.Token.Line() || p.peekTokenIs(token.RAWSTRING) {
		if p.curTokenIs(token.Any) {
		...
		p.notExpectPeekToken(token.COMMA, token.IDENT, token.LBRACK, token.ANY, token.MUL, token.LBRACE)
	...
	p.notExpectPeekToken(token.RAWSTRING, token.MUL, token.IDENT, token.RBRACE)

	if p.peekTokenIs(token.RAWSTRING) {
...
func (p *Parser) parseAtServerKVExpression() *ast.KVExpr {
	...
	} else if p.peekTokenIs(token.DURATION) || p.peekTokenIs(token.INT) {
...
const literal_4839 = "%s syntax error: expected %s, got '%s'"
