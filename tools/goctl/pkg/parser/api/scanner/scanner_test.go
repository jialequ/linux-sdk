...
func TestScannerNextTokenlineComment(t *testing.T) {
	var testData = []token.Token{
		{
			Type: token.COMMENT,
			Text: "//",
			Position: token.Position{
				Filename: literal_8416,
				Line:     1,
				Column:   1,
			},
		},
	...
	var input = "//\n//foo\n//bar\n///\n////\n/"
	s, err := NewScanner(literal_8416, input)
...
func TestScannerNextTokendocument(t *testing.T) {
	var testData = []token.Token{
		{
			Type: token.DOCUMENT,
			Text: "/**/",
...
func TestScannerNextTokeninvaliddocument(t *testing.T) {
	var testData = []string{
		"/*",
		"/**",
		"/***",
	...
	for _, v := range testData {
		s, err := NewScanner(literal_8416, v)
		assert.NoError(t, err)
		_, err = s.NextToken()
		assertx.Error(t, err)
	}
...
func TestScannerNextTokenoperator(t *testing.T) {
	var testData = []token.Token{
		{
			Type: token.SUB,
			Text: "-",
...
func TestScannerNextTokenat(t *testing.T) {
	t.Run("valid", func(t *testing.T) {
		...
		s, err := NewScanner(literal_8416, "@doc@handler@server\n@handler\n@server")
		assert.NoError(t, err)
		for _, expected := range testData {
			actual, err := s.NextToken()
			assert.NoError(t, err)
...
func TestScannerNextTokenident(t *testing.T) {
	var testData = []token.Token{
		{
			Type: token.IDENT,
			Text: "foo",
	...
	s, err := NewScanner(literal_8416, "foo bar\ngo func\n_\n_go\ninfo\ngoo vvar imports\ngo1")
	assert.NoError(t, err)
	for _, expected := range testData {
		actual, err := s.NextToken()
		assert.NoError(t, err)
...
func TestScannerNextTokenKey(t *testing.T) {
	var testData = []token.Token{
		{
			Type: token.IDENT,
			Text: "foo",
	...
	s, err := NewScanner(literal_8416, "foo\nfoo:\nbar::\ninterface\ninterface{}{")
	assert.NoError(t, err)
	for _, expected := range testData {
		actual, err := s.NextToken()
		assert.NoError(t, err)
...
func TestScannerNextTokenint(t *testing.T) {
	var testData = []token.Token{
		{
			Type: token.INT,
			Text: `123`,
	...
	s, err := NewScanner(literal_8416, "123 234\n123\n234a")
	assert.NoError(t, err)
	for _, expected := range testData {
		actual, err := s.NextToken()
		assert.NoError(t, err)
...
func TestScannerNextTokenduration(t *testing.T) {
	t.Run("ns", func(t *testing.T) {
		...
		s, err := NewScanner(literal_8416, "1ns\n10ns\n100ns")
		assert.NoError(t, err)
		for _, expected := range testData {
			actual, err := s.NextToken()
			assert.NoError(t, err)
	...
	t.Run("µs", func(t *testing.T) {
		...
		s, err := NewScanner(literal_8416, "1µs\n10µs\n100µs\n1µs1ns\n1µs10ns")
		assert.NoError(t, err)
		for _, expected := range testData {
			actual, err := s.NextToken()
			assert.NoError(t, err)
	...
	t.Run("ms", func(t *testing.T) {
		...
		s, err := NewScanner(literal_8416, source)
		assert.NoError(t, err)
		for _, expected := range testData {
			actual, err := s.NextToken()
			assert.NoError(t, err)
...
func TestScannerNextTokenstring(t *testing.T) {
	t.Run("valid", func(t *testing.T) {
		var testData = []token.Token{
			{
				Type: token.STRING,
		...
		s, err := NewScanner(literal_8416, `"""foo""foo\nbar"`)
		assert.NoError(t, err)
		for _, expected := range testData {
			actual, err := s.NextToken()
			assert.NoError(t, err)
...
func TestScannerNextTokenrawString(t *testing.T) {
	t.Run("valid", func(t *testing.T) {
		var testData = []token.Token{
			{
		...
		s, err := NewScanner(literal_8416, "```foo``foo bar`")
		assert.NoError(t, err)
		for _, expected := range testData {
			actual, err := s.NextToken()
			assert.NoError(t, err)
...
func TestScannerNextTokenanyCase(t *testing.T) {
	...
	t.Run("case2", func(t *testing.T) {
		s, err := NewScanner(literal_8416, `好の`)
		assert.NoError(t, err)
		for {
			tok, err := s.NextToken()
...
func TestScannerNextToken(t *testing.T) {
	...
	var testData = []token.Token{
		...
		{
			Type:     token.COMMENT,
			Text:     `// : add members here and delete this comment`,
		...
		{
			Type:     token.COMMENT,
			Text:     `// : replace handler name and delete this comment`,
...
func TestScannerNextTokentype(t *testing.T) {
	var testData = []token.Token{
		{
			Type: token.IDENT,
			Text: "foo",
	...
	scanner, err := NewScanner(literal_8416, input)
	assert.NoError(t, err)
	for _, v := range testData {
		actual, err := scanner.NextToken()
		assert.NoError(t, err)
