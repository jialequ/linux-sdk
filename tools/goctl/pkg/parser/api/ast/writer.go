...
func transfer2TokenNode(node Node, isChild bool, opt ...tokenNodeOption) *TokenNode { //NOSONAR
	...
	case *SliceDataType:
		copyOpt = append(copyOpt, withTokenNodePrefix(NilIndent))
		var tn1 *TokenNode
		tn1 = transferTokenNode(val.LBrack, copyOpt...)
		if option.ignoreHeadComment {
			tn1.HeadCommentGroup = nil
		}
		if option.ignoreLeadingComment {
			tn1.LeadingCommentGroup = nil
		}
		val.isChild = isChild
		val.LBrack = tn1
	case *StructDataType:
