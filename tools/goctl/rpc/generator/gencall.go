...
func (g *Generator) genCallGroup(ctx DirContext, proto parser.Proto, cfg *conf.Config) error {
	...
	for _, service := range proto.Service {
		childPkg, _ := dir.GetChildPackage(service.Name)

		callFilename, _ := format.FileNamingFormat(cfg.NamingFormat, service.Name)
		...
		functions, _ := g.genFunction(proto.PbPackage, serviceName, service, isCallPkgSameToGrpcPkg)

		iFunctions, _ := g.getInterfaceFuncs(proto.PbPackage, service, isCallPkgSameToGrpcPkg)

		text, _ := pathx.LoadTemplate(category, callTemplateFile, callTemplateText)
