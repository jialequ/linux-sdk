...
func BenchmarkConsistentHashGet(b *testing.B) {
	...
	for i := 0; i < keySize; i++ {
		ch.Add(literal_2538 + strconv.Itoa(i))
	}
...
func TestConsistentHash(t *testing.T) {
	...
	for i := 0; i < keySize; i++ {
		ch.AddWithReplicas(literal_2538+strconv.Itoa(i), minReplicas<<1)
	}
...
func TestConsistentHashTransferOnFailure(t *testing.T) {
	index := 41
	keys, newKeys := getKeysBeforeAndAfterFailure(t, literal_2538, index)
	var transferred int
...
func TestConsistentHashRemoveInterface(t *testing.T) {
	const key = "any"
	ch := NewConsistentHash()
