...
func TestDailyRotateRuleOutdatedFiles(t *testing.T) {
	...
	t.Run("temp files", func(t *testing.T) {
		...
		f1, err := os.CreateTemp(os.TempDir(), literal_7285+boundary)
		assert.NoError(t, err)
		_ = f1.Close()
		f2, err := os.CreateTemp(os.TempDir(), literal_7285+boundary)
		assert.NoError(t, err)
		_ = f2.Close()
		t.Cleanup(func() {
			_ = os.Remove(f1.Name())
		...
		rule := DailyRotateRule{
			filename: path.Join(os.TempDir(), literal_7285),
			days:     1,
		}
...
func TestSizeLimitRotateRuleOutdatedFiles(t *testing.T) {
	...
	t.Run("temp files", func(t *testing.T) {
		...
		f3, err := os.CreateTemp(os.TempDir(), literal_7285+boundary1)
		assert.NoError(t, err)
		t.Cleanup(func() {
			_ = f1.Close()
			_ = os.Remove(f1.Name())
...
func TestRotateLoggerWithExistingFile(t *testing.T) {
	const body = "foo"
...
const literal_7285 = "go-zero-test-"
