...
func TestWriteJson(t *testing.T) {
	...
	assert.Contains(t, buf.String(), literal_1498)

	buf.Reset()
	writeJson(nil, make(chan int))
	assert.Contains(t, buf.String(), literal_8601)
...
func (h hardToWriteWriter) Write(_ []byte) (_ int, _ error) {
	return 0, errors.New(literal_1498)
}

const literal_1498 = "write error"

const literal_8601 = "unsupported type"
