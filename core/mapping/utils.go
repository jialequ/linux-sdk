...
func implicitValueRequiredStruct(tag string, tp reflect.Type) (bool, error) {
	...
	for i := 0; i < numFields; i++ {
		...
		_, opts, _ := parseKeyAndOptions(tag, childField)
...
func parseNumberRange(str string) (*numberRange, error) {
	leftInclude, _ := isLeftInclude(str[0])
	...
	rightInclude, _ := isRightInclude(str[len(str)-1])
	...
	var left float64
	if len(fields[0]) > 0 {
		left, _ = strconv.ParseFloat(fields[0], 64)
	} else {
