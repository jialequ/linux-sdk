...
func TestlogDurationWithDoc(t *testing.T) {
	buf := logtest.NewCollector(t)
	buf.Reset()
	...
	buf.Reset()
	logDurationWithDocs(context.Background(), "foo", "bar", timex.Now()-slowThreshold.Load()*2, nil, literal_3129)
	assert.Contains(t, buf.String(), "foo")
	assert.Contains(t, buf.String(), "bar")
	assert.Contains(t, buf.String(), "slow")
	...
	buf.Reset()
	logDurationWithDocs(context.Background(), "foo", "bar", timex.Now(), nil, literal_3129)
	assert.Contains(t, buf.String(), "foo")
	assert.Contains(t, buf.String(), "bar")
	assert.Contains(t, buf.String(), "json")
	...
	buf.Reset()
	logDurationWithDocs(context.Background(), "foo", "bar", timex.Now(), errors.New("bar"), literal_3129)
	assert.Contains(t, buf.String(), "foo")
	assert.Contains(t, buf.String(), "bar")
	assert.Contains(t, buf.String(), "fail")
	assert.Contains(t, buf.String(), "json")
