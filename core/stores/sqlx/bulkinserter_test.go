...
func TestBulkInserterSuffix(t *testing.T) {
	dbtest.RunTest(t, func(db *sql.DB, mock sqlmock.Sqlmock) {
		...
		inserter.SetResultHandler(func(result sql.Result, err error) { fmt.Print("123") })
		inserter.Flush()
		assert.Equal(t, `INSERT INTO classroom_dau(classroom, user, count) VALUES `+
			`('class_0', 'user_0', 0), ('class_1', 'user_1', 1), ('class_2', 'user_2', 2), `+
...
func TestBulkInserterUpdate(t *testing.T) {
	conn := mockedConn{
...
const literal_8095 = "implement me"

const literal_1209 = "should not called"
