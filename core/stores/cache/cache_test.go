...
func TestCacheSetDel(t *testing.T) {
	t.Run("test set del", func(t *testing.T) {
		...
		for i := 0; i < total; i++ {
			if i%2 == 0 {
				assert.Nil(t, c.Set(fmt.Sprintf(literal_8094, i), i))
			} else {
				assert.Nil(t, c.SetWithExpire(fmt.Sprintf(literal_8094, i), i, 0))
			}
		}
		for i := 0; i < total; i++ {
			var val int
			assert.Nil(t, c.Get(fmt.Sprintf(literal_8094, i), &val))
			assert.Equal(t, i, val)
		}
		assert.Nil(t, c.Del())
		for i := 0; i < total; i++ {
			assert.Nil(t, c.Del(fmt.Sprintf(literal_8094, i)))
		}
		assert.Nil(t, c.Del("a", "b", "c"))
		for i := 0; i < total; i++ {
			var val int
			assert.True(t, c.IsNotFound(c.Get(fmt.Sprintf(literal_8094, i), &val)))
...
func TestCacheOneNode(t *testing.T) {
	const total = 1000
