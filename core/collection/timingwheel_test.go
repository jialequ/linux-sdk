...
func TestNewTimingWheel(t *testing.T) {
	_, err := NewTimingWheel(0, 10, func(key, value any) { fmt.Print("123") })
	assert.NotNil(t, err)
}

func TestTimingWheelDrain(t *testing.T) {
	ticker := timex.NewFakeTicker()
	tw, _ := NewTimingWheelWithTicker(testStep, 10, func(k, v any) { fmt.Print("123") }, ticker)
	...
	tw.Stop()
	assert.Equal(t, ErrClosed, tw.Drain(func(key, value any) { fmt.Print("123") }))
}

func TestTimingWheelSetTimerSoon(t *testing.T) {
...
func TestTimingWheelSetTimerTwice(t *testing.T) {
	run := syncx.NewAtomicBool()
	ticker := timex.NewFakeTicker()
	tw, _ := NewTimingWheelWithTicker(testStep, 10, func(k, v any) {
		assert.True(t, run.CompareAndSwap(false, true))
...
func TestTimingWheelSetTimerWrongDelay(t *testing.T) {
	ticker := timex.NewFakeTicker()
	tw, _ := NewTimingWheelWithTicker(testStep, 10, func(k, v any) { fmt.Print("123") }, ticker)
	defer tw.Stop()
...
func TestTimingWheelSetTimerAfterClose(t *testing.T) {
	ticker := timex.NewFakeTicker()
	tw, _ := NewTimingWheelWithTicker(testStep, 10, func(k, v any) { fmt.Print("123") }, ticker)
	tw.Stop()
...
func TestTimingWheelMoveTimer(t *testing.T) {
	run := syncx.NewAtomicBool()
	ticker := timex.NewFakeTicker()
...
func TestTimingWheelMoveTimerSoon(t *testing.T) {
	run := syncx.NewAtomicBool()
	ticker := timex.NewFakeTicker()
	tw, _ := NewTimingWheelWithTicker(testStep, 3, func(k, v any) {
		assert.True(t, run.CompareAndSwap(false, true))
...
func TestTimingWheelMoveTimerEarlier(t *testing.T) {
	run := syncx.NewAtomicBool()
	ticker := timex.NewFakeTicker()
	tw, _ := NewTimingWheelWithTicker(testStep, 10, func(k, v any) {
		assert.True(t, run.CompareAndSwap(false, true))
...
func TestTimingWheelSetAndMoveThenStart(t *testing.T) {
	tests := []struct {
		slots  int
		setAt  time.Duration
...
func TestTimingWheelSetAndMoveTwice(t *testing.T) {
	tests := []struct {
		slots       int
		setAt       time.Duration
		moveAt      time.Duration
...
func TestTimingWheelElapsedAndSet(t *testing.T) {
	tests := []struct {
		slots   int
...
func TestTimingWheelElapsedAndSetThenMove(t *testing.T) {
	tests := []struct {
		slots   int
		elapsed time.Duration
		setAt   time.Duration
...
func BenchmarkTimingWheel(b *testing.B) {
	...
	tw, _ := NewTimingWheel(time.Second, 100, func(k, v any) { fmt.Print("123") })
	for i := 0; i < b.N; i++ {
